...
class Front3D_Recon_Dataset(Dataset):
    def __init__(self, config, mode, unlabel):
        ...
        if self.unlabel:
            dataset_name = self.config['data']['unlabeled_dataset']       # now, just for front3d
            split_dir = self.config['data']['unlabeled_split_dir']
            batch_size = config['data']['batch_size']['unlabel'],
        else:
        ...
        if self.use_diffu_prior:
            self.diffu_prior_path = 'ssr/depth_anything/'
    ...
    def __getitem__(self, index):
        ...
        if self.config['data']['load_dynamic'] == True:
            if self.unlabel:
                img_path = os.path.join(self.config['data']['unlabeled_data_path'], imgid)
            else:
...
def Front3D_Recon_dataloader(config, mode='train', unlabel=False):
    # 根据 unlabel 的值设置 batch_size
    if unlabel:
        batch_size = config['data']['batch_size']['unlabel']
    else:
    ...
    dataloader = DataLoader(
        dataset=Front3D_Recon_Dataset(config, mode, unlabel),
