...
class MonoSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth, epoch, label=True):
        ...
        if label:
            ...
            rgb_gt = ground_truth['rgb_values'].reshape(-1, self.num_pixels, 3)
        # # only supervised the foreground normal
        if self.vis_mask_loss:
            # 获取前景可见性 mask
            if label:
                vis_mask = ground_truth['vis_pixel'].cuda()  # 使用 ground_truth 提供的前景 mask
            else:
                vis_mask = torch.ones(rgb_pred.shape[0], rgb_pred.shape[1], dtype=bool).cuda()  # 全图可见
        else:
        ...
        if label:
            ...
            ray_mask_gt = ground_truth['ray_mask'].reshape(-1, self.num_pixels)
        ...
        if self.use_depth:
            if label:
                ...
                depth_gt = ground_truth['depth_values']
                depth_gt = depth_gt.reshape(-1, self.num_pixels, 1) 
        ...
        if self.use_normal:
            if label:
                ...
                normal_gt = ground_truth['normal_map']
                normal_gt = normal_gt.reshape(-1, self.num_pixels, 3)               # [B, Num_pixels, 3]
        ...
        if self.use_sdf:
            ...
            if self.add_bdb3d_points:       # add sdf loss of add points
                ...
                if label:
                    ...
                    add_sdf_gt = ground_truth['add_sdf']
